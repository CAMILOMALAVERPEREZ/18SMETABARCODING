...
OUTBASE="$ROOT/data/07_non_inoculated_samples/asv_audit_genus_13samples"
...
# --------------------------------------------------
# Script Python reusable para construir tabla ASV
# --------------------------------------------------
cat > "$OUTBASE/.build_asv_audit_generic.py" << 'PY'
#!/usr/bin/env python3
import csv
import sys
...
if len(sys.argv) != 5:
    print("Uso: build_asv_audit_generic.py <BASE_DIR> <DB_NAME> <GENUS_LEVEL> <OUT_TSV>")
    sys.exit(1)

base_dir = Path(sys.argv[1])
db_name = sys.argv[2]
genus_level = int(sys.argv[3])
...
with open(out_tsv, "w", newline="", encoding="utf-8") as f_out:
    ...
    for line in lines[1:]:
        ...
        abunds = parts[1:]
        tax = tax_map.get(asv, "")
        genero = extraer_nivel(tax, genus_level)
        seq = seq_map.get(asv, "")
...
chmod +x "$OUTBASE/.build_asv_audit_generic.py"
...
process_one() {
  ...
  # 4. exportar taxonomía
  qiime tools export \
    --input-path "$TAX_IN" \
    --output-path "$OUTDIR/export_taxonomy"

  # 5. exportar secuencias
  qiime tools export \
    --input-path "$SEQS_IN" \
    --output-path "$OUTDIR/export_seqs"

  # 6. construir tabla ASV auditoría
  python3 "$OUTBASE/.build_asv_audit_generic.py" \
    "$OUTDIR" "$DB" "$GENUS_LEVEL" "$OUTDIR/ASV_audit_genus_${DB}_${PIPE}.tsv"
}

# --------------------------------------------------
# PR2: género = posición 8
# --------------------------------------------------
...
# --------------------------------------------------
# SILVA138custom: género = posición 6
# tabla desde 04_dada2_asvs, taxonomía y seqs desde 05_taxonomy_SILVA138_custom
# --------------------------------------------------
for P in "${PIPELINES[@]}"; do
  process_one \
    "SILVA138custom" \
...
# --------------------------------------------------
# Unir todas las tablas en una tabla maestra final
# --------------------------------------------------
MASTER="$OUTBASE/ASV_audit_genus_MASTER_13samples.tsv"

python3 - << 'PY'
from pathlib import Path
import csv

root = ROOT / "data" / "07_non_inoculated_samples" / "asv_audit_genus_13samples"
out_file = root / "ASV_audit_genus_MASTER_13samples.tsv"

files = sorted(root.glob("*/*/ASV_audit_genus_*.tsv"))
        ...
        with open(f, newline="", encoding="utf-8") as fin:
            ...
            for row in reader:
                row_out = {
                    ...
                    "genero": row.get("genero_PR2", row.get("genero_SILVA132", row.get("genero_SILVA138custom", ""))),
...
echo
echo "[DONE] Auditoría ASV de género completada."
